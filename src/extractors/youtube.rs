...
impl Extractor for Annie {
    ...
    fn real_url(value: &Value) -> Option<Url> {
        let (_, stream) = super::search_by_keys(&value[0]["streams"], &Self::DISPLAYS)?;
        let video_url = value_to_string!(stream["parts"][0]["url"])?;
        let audio_url = value_to_string!(stream["parts"][1]["url"])?;
